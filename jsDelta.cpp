											...
											MAutoCLEvent &outputEvent)
{
	...
	for (unsigned int i = 0; i < smoothIterations; ++i)
	{
		if (i == 0)
		{
			...
			err = clEnqueueNDRangeKernel(MOpenCLInfo::getMayaDefaultOpenCLCommandQueue(),
										kernelSmooth_.get(),
										1,
										NULL,
										&pingGlobalWorkSize,
	...
	err = clEnqueueNDRangeKernel(MOpenCLInfo::getMayaDefaultOpenCLCommandQueue(), // cl_command_queue, in maya wrapper
...
bool JSDeltaGPUInfo::validateNodeValues(MDataBlock &data,
						const MEvaluationNode &evaluationNode,
						const MPlug &plug,
...
bool JSDeltaGPUInfo::validateNodeInGraph(MDataBlock &data,
						const MEvaluationNode &evaluationNode,
						const MPlug &plug,
						MStringArray *messages)
{
